...
export const CitizenDashboard: React.FC = () => {
  ...
  const stats = [
    { label: t('common.totalReports'), value: myComplaints.length, icon: AlertCircle, color: 'text-[#000000]', bg: 'bg-gray-100/50' },
  ...
  return (
    <div className="p-4 md:p-8 lg:p-10 space-y-6 md:space-y-8 max-w-[1600px] mx-auto overflow-hidden">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="text-3xl md:text-4xl font-black text-slate-900 tracking-tight mb-2">
            Hey, <span className="text-[#000000]">{user?.name.split(' ')[0]}</span>! 👋
          </h1>
        ...
        <Link to="/report" className="w-full md:w-auto">
          <Button className="w-full h-14 px-8 text-base shadow-lg shadow-black/10" variant="primary">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        ...
        <Card className="col-span-1 sm:col-span-2 lg:col-span-4 p-6 md:p-8 bg-slate-50/50 border-2 border-white shadow-premium">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-6">
            {[
              ...
              <motion.div 
                key={item.status} 
                className="flex flex-col items-center p-4 md:p-6 rounded-[2rem] md:rounded-[2.5rem] bg-white border border-slate-100 hover:shadow-premium hover:-translate-y-1 transition-all group lg:aspect-square justify-center"
        ...
        <div className="col-span-1 sm:col-span-2 lg:col-span-4 grid grid-cols-1 lg:grid-cols-3 gap-6">
            <Card className="lg:col-span-1 p-6 md:p-8 flex flex-col bg-gray-50 border-gray-200/50 text-slate-900 border relative overflow-hidden">
                <div className="absolute top-0 right-0 w-32 h-32 bg-gray-400 opacity-5 blur-3xl rounded-full" />
                ...
                <div className="space-y-4 flex-1 relative z-10">
                    <Link to="/map" className="w-full flex items-center gap-4 p-5 rounded-3xl bg-white hover:bg-slate-50 border border-slate-200/50 shadow-sm transition-all text-left group">
                        <div className="w-12 h-12 rounded-2xl bg-gray-100 flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform">
                    ...
                    <Link to="/maintenance" className="w-full flex items-center gap-4 p-5 rounded-3xl bg-white hover:bg-slate-50 border border-slate-200/50 shadow-sm transition-all text-left group">
                        <div className="w-12 h-12 rounded-2xl bg-[#000000] flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform">
                ...
                <div className="mt-8 relative z-10 text-center">
                    <p className="text-xs font-bold text-[#374151]/60 italic">"Working together for a cleaner city"</p>
                </div>
            </Card>

            <Card className="lg:col-span-2 p-6 md:p-8 flex flex-col">
                <div className="flex items-center justify-between mb-8">
                    <h3 className="text-xl font-black text-slate-900 tracking-tight">{t('dashboard.recentComplaints')}</h3>
                    <Link to="/my-complaints">
                        <Button variant="ghost" size="sm" className="font-black text-[#000000] hover:text-[#1F2937] hover:bg-gray-50 rounded-full flex items-center">
                            {t('common.viewAll')}
                    ...
                    {myComplaints.slice(0, 2).map((complaint) => (
                        <Link key={complaint.id} to={`/complaints/${complaint.id}`} className="group">
                            <div className="bg-slate-50/50 rounded-[2.5rem] border border-slate-100 overflow-hidden group-hover:shadow-premium group-hover:-translate-y-1 transition-all duration-300">
                                ...
                                <div className="p-6">
                                    <h4 className="font-bold text-slate-900 text-lg mb-2 group-hover:text-[#374151] transition-colors line-clamp-1">{complaint.title}</h4>
